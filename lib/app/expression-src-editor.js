...
var padding = function (string, pos) {
      ...
      if (pos > 0 && pos < string.length - 1) {
        var testSub = string[pos - 1] + '@@' + string[pos + 1],
            re = /[^\W\d]@@[\w]/;
        console.log('Testing ' + testSub + '. pad? ' + !!testSub.match(re));
        return !!testSub.match(re) ? " " : "";
      }
      return "";
    };
...
_.extend(TokenDeleter.prototype, {
  ...
  _deleteMatchingParens: function (node, position, string) {
    ...
    var removeParens = function remove (open, close, isOpening) {
      var openPadding = padding(string, open),
          closePadding = padding(string, close);
      ...
      var cursorPosition = isOpening ? open : openPadding.length + close - 1;
  ...
  visitUnaryOperation: function(node, position, ctx) { 
    ...
    return {
      string: string.substring(0, insertionPoint) + string.substring(excisionPoint),
  ...
  visitBinaryOperator: function(node, position, ctx) { 
    ...
    return {
      string: string.substring(0, node._offsetLiteralFirst) + string.substring(node._offsetLiteralLast + 1),
      cursorPosition: node.left.offsetLast + 1
    };
  },
  ...
  visitInteger: function(node, position, ctx) { 
    ...
    var newString = string.substring(0, position) + string.substring(position + 1, string.length);
    return newString;
  },
  ...
  visitIdentifier: function(node, position, ctx) { 
    ...
    var newString = string.substring(0, start) + string.substring(end, string.length);
  ...
  visitUnitLiteral: function (node, position, ctx) { 
    ...
    if (end <= string.length) {
      ...
      // this needs to take into account adjacent tokens.
      var testSub = string.substring(0, start) + '@@' + string[end],
              re = /[^\W\d]@@[\w]/;
      if (start !== node.offsetFirst && testSub.match(re)) {
        start = node.offsetFirst;
      }
    }
    return { 
